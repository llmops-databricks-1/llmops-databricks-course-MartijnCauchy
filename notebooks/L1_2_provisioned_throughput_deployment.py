...
schema = "arxiv"
BUDGET_POLICY_ID = "501f8711-f61e-4f1f-bffd-6d474dd2a3f0"
# COMMAND ----------
...
# MAGIC %md
# MAGIC ### Step 2: Create or update the endpoint with provisioned throughput
# MAGIC
...
# Drop leftover inference table from previous runs so create() doesn't fail
spark.sql(f"DROP TABLE IF EXISTS {catalog}.{schema}.provisioned_throughput_monitoring_payload")
logger.info("Dropped existing inference table (if any)")

# Idempotent create/update: update config if endpoint exists, create if it doesn't
try:
    w.serving_endpoints.get(ENDPOINT_NAME)
...
response = client.chat.completions.create(
    ...
    messages=[
        ...
        {"role": "user", "content": "Give me a recipe for something i can cook with these ingredients: Quinoa, lentils, sweet potato, chickpeas, oats. It should be one dish, not a menu."}
    ],
    max_tokens=500,
    temperature=0.7
)
